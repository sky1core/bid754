...
func TestValueTypesPreserveEveryRawBit(t *testing.T) {
	t.Run("decimal32", func(t *testing.T) {
		for bit := 0; bit < 32; bit++ {
			want := uint32(1) << bit
			if got := Decimal32BIDFromBits(want).ToUint32(); got != want {
	...
	t.Run("decimal64", func(t *testing.T) {
		for bit := 0; bit < 64; bit++ {
			want := uint64(1) << bit
			if got := Decimal64BIDFromBits(want).ToUint64(); got != want {
	...
	t.Run("decimal128", func(t *testing.T) {
		for bit := 0; bit < 128; bit++ {
			...
			if got := Decimal128BIDFromBytes(want).ToBytes(); got != want {
