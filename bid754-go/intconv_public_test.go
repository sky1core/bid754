...
func intConvInputs32(t *testing.T) []intConvInput32 {
	...
	return []intConvInput32{
		...
		{name: "int32 overflow", value: mustDecimal32BID(t, "2147484e3")},
...
func intConvInputs64(t *testing.T) []intConvInput64 {
	...
	return []intConvInput64{
		...
		// Exact Decimal64 value just above MaxInt64, written with a coefficient
		// and quantum the format can preserve without constructor coercion.
		{name: "int64 overflow", value: mustDecimal64BID(t, "9223372036854776e3")},
...
// assertNoPanic, deferred at the top of a leaf subtest, converts a panic on an
// invalid RoundingMode into a test failure with a clear message. docs/SPEC.md
// forbids any public API path from crashing the process through panic/trap on
// unsupported input, so a panic here is a contract violation, not a pass.
func assertNoPanic(t *testing.T) {
	t.Helper()
	if r := recover(); r != nil {
		t.Fatalf("panicked on invalid RoundingMode: %v", r)
	}
}

func assertInvalidFlag(t *testing.T, f ExceptionFlags) {
	t.Helper()
	if f&FlagInvalidOperation == 0 {
		t.Errorf("flags = %s, want FlagInvalidOperation raised", f)
	}
}
...
func TestInvalidRoundingModeRejectedNotPanics(t *testing.T) {
	for _, bad := range []RoundingMode{RoundingMode(99), RoundingMode(-1)} {
		bad := bad
		t.Run(fmt.Sprintf("mode%d", int(bad)), func(t *testing.T) {
			d32 := mustDecimal32BID(t, "1.5")
			d64 := mustDecimal64BID(t, "1.5")
			d128 := mustDecimal128BID(t, "1.5")

			// Integer conversions (value+flags channel): the value must be the
			// NaN-input sentinel, not a silently-rounded valid integer.
			t.Run("Decimal32.ConvertToInt32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d32.ConvertToInt32(bad)
				assertInvalidFlag(t, f)
				if v != math.MinInt32 {
					t.Errorf("value = %d, want NaN-input sentinel %d", v, int32(math.MinInt32))
				}
			})
			t.Run("Decimal32.ConvertToUint32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d32.ConvertToUint32(bad)
				assertInvalidFlag(t, f)
				if v != uint32(0x80000000) {
					t.Errorf("value = %#x, want NaN-input sentinel 0x80000000", v)
				}
			})
			t.Run("Decimal32.ConvertToInt64Exact", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d32.ConvertToInt64Exact(bad)
				assertInvalidFlag(t, f)
				if v != math.MinInt64 {
					t.Errorf("value = %d, want NaN-input sentinel %d", v, int64(math.MinInt64))
				}
			})
			t.Run("Decimal64.ConvertToInt32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d64.ConvertToInt32(bad)
				assertInvalidFlag(t, f)
				if v != math.MinInt32 {
					t.Errorf("value = %d, want NaN-input sentinel %d", v, int32(math.MinInt32))
				}
			})
			t.Run("Decimal128.ConvertToInt32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d128.ConvertToInt32(bad)
				assertInvalidFlag(t, f)
				if v != math.MinInt32 {
					t.Errorf("value = %d, want NaN-input sentinel %d", v, int32(math.MinInt32))
				}
			...
			// Binary (float) conversions: NaN result + invalid flag.
			t.Run("Decimal32.ToBinary32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d32.ToBinary32(bad)
				assertInvalidFlag(t, f)
				if !math.IsNaN(float64(v)) {
					t.Errorf("value = %v, want NaN", v)
				}
			})
			t.Run("Decimal64.ToBinary64", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d64.ToBinary64(bad)
				assertInvalidFlag(t, f)
				if !math.IsNaN(v) {
					t.Errorf("value = %v, want NaN", v)
				}
			})
			t.Run("Decimal128.ToBinary128", func(t *testing.T) {
				defer assertNoPanic(t)
				_, f := d128.ToBinary128(bad)
				assertInvalidFlag(t, f)
			})

			// Cross-width decimal narrowing: canonical quiet NaN + invalid flag.
			t.Run("Decimal64.ToDecimal32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d64.ToDecimal32(bad)
				assertInvalidFlag(t, f)
				if !v.IsNaN() {
					t.Errorf("value = %s, want NaN", v.String())
				}
			})
			t.Run("Decimal128.ToDecimal64", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := d128.ToDecimal64(bad)
				assertInvalidFlag(t, f)
				if !v.IsNaN() {
					t.Errorf("value = %s, want NaN", v.String())
				}
			...
			// Integer-to-decimal constructors: canonical quiet NaN + invalid.
			t.Run("NewDecimal32FromInt32", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := NewDecimal32FromInt32(1, bad)
				assertInvalidFlag(t, f)
				if !v.IsNaN() {
					t.Errorf("value = %s, want NaN", v.String())
				}
			})
			t.Run("NewDecimal32FromUint64", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := NewDecimal32FromUint64(1, bad)
				assertInvalidFlag(t, f)
				if !v.IsNaN() {
					t.Errorf("value = %s, want NaN", v.String())
				}
			})
			t.Run("NewDecimal64FromInt64", func(t *testing.T) {
				defer assertNoPanic(t)
				v, f := NewDecimal64FromInt64(1, bad)
				assertInvalidFlag(t, f)
				if !v.IsNaN() {
					t.Errorf("value = %s, want NaN", v.String())
				}
...
// TestValidRoundingModesRemainAccepted pins that the five defined rounding
// modes are unaffected by the invalid-mode rejection path: a finite in-range
// operation must not raise FlagInvalidOperation and must not return NaN.
func TestValidRoundingModesRemainAccepted(t *testing.T) {
	valid := []RoundingMode{
		RoundNearestEven,
		RoundNearestAway,
		RoundTowardZero,
	...
	for _, m := range valid {
		...
		t.Run(m.String(), func(t *testing.T) {
			...
			if got, f := d32.ToBinary64(m); math.IsNaN(got) || f&FlagInvalidOperation != 0 {
				t.Errorf("ToBinary64 = %v flags=%s, want finite non-invalid", got, f)
			}
		})
	}
}
