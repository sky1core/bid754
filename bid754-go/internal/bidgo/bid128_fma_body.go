...
func bid_fma_delta_ge_zero(
	...
	if p34 <= delta-1 || // Case (1')
			...
			if rnd_mode == BID_ROUNDING_TO_NEAREST {
				...
				*pfpsf |= (BID_INEXACT_EXCEPTION | BID_OVERFLOW_EXCEPTION)
				...
				if scale == 0 {
					...
					if q3 <= 19 {
						if scale <= 19 {
							*res = __mul_64x64_to_128(C3.lo, bid_ten2k64[scale])
						} else {
							*res = __mul_64x128_to_128(C3.lo, bid_ten2k128[scale-20])
						}
					} else {
		...
		if q3 < p34 {
			...
			e3 = e3 - scale
			res.hi = z_sign | (z_exp & MASK_EXP_128) | res.hi
			if scale+q3 < p34 {
		...
		if (p_sign != z_sign) && (delta == (q3 + scale + 1)) {
			if (q3 <= 19 && C3.lo != bid_ten2k64[q3-1]) ||
				...
				if q4 == 1 {
					...
					if q4 <= 18 {
						R64 = bid_round64_2_18(q4, q4-1, C4.w0, &incr_exp,
							&is_midpoint_lt_even, &is_midpoint_gt_even,
							&is_inexact_lt_midpoint,
							&is_inexact_gt_midpoint)
					} else if q4 <= 38 {
		...
		if (e3 == expmin128 && (q3+scale) < p34) ||
			(e3 == expmin128 && (q3+scale) == p34 &&
				(res.hi&MASK_COEFF128) == 0x0000314dc6448d93 &&
		...
		*ptr_is_inexact_gt_midpoint = is_inexact_gt_midpoint
		...
		gt_half_ulp = 0
		if q4 <= 19 {
			if C4.w0 < bid_midpoint64[q4-1] {
				lt_half_ulp = 1
			} else if C4.w0 == bid_midpoint64[q4-1] {
				eq_half_ulp = 1
			} else {
				gt_half_ulp = 1
			}
		} else if q4 <= 38 {
			if C4.w2 == 0 && (C4.w1 < bid_midpoint128[q4-20].hi ||
				(C4.w1 == bid_midpoint128[q4-20].hi &&
					C4.w0 < bid_midpoint128[q4-20].lo)) {
				lt_half_ulp = 1
			} else if C4.w2 == 0 && C4.w1 == bid_midpoint128[q4-20].hi &&
				C4.w0 == bid_midpoint128[q4-20].lo {
				eq_half_ulp = 1
			} else {
				gt_half_ulp = 1
			...
			if C4.w3 == 0 && (C4.w2 < bid_midpoint192[q4-39].w2 ||
				(C4.w2 == bid_midpoint192[q4-39].w2 &&
					C4.w1 < bid_midpoint192[q4-39].w1) ||
				(C4.w2 == bid_midpoint192[q4-39].w2 &&
					C4.w1 == bid_midpoint192[q4-39].w1 &&
					C4.w0 < bid_midpoint192[q4-39].w0)) {
				lt_half_ulp = 1
			} else if C4.w3 == 0 && C4.w2 == bid_midpoint192[q4-39].w2 &&
				C4.w1 == bid_midpoint192[q4-39].w1 &&
				C4.w0 == bid_midpoint192[q4-39].w0 {
				eq_half_ulp = 1
			} else {
			...
			if C4.w3 < bid_midpoint256[q4-59].w3 ||
				(C4.w3 == bid_midpoint256[q4-59].w3 &&
					C4.w2 < bid_midpoint256[q4-59].w2) ||
				(C4.w3 == bid_midpoint256[q4-59].w3 &&
					C4.w2 == bid_midpoint256[q4-59].w2 &&
					C4.w1 < bid_midpoint256[q4-59].w1) ||
				(C4.w3 == bid_midpoint256[q4-59].w3 &&
					C4.w2 == bid_midpoint256[q4-59].w2 &&
					C4.w1 == bid_midpoint256[q4-59].w1 &&
					C4.w0 < bid_midpoint256[q4-59].w0) {
				lt_half_ulp = 1
			} else if C4.w3 == bid_midpoint256[q4-59].w3 &&
				C4.w2 == bid_midpoint256[q4-59].w2 &&
				C4.w1 == bid_midpoint256[q4-59].w1 &&
				C4.w0 == bid_midpoint256[q4-59].w0 {
				eq_half_ulp = 1
		...
		if p_sign == z_sign {
			if lt_half_ulp != 0 {
				res.hi = z_sign | (z_exp & MASK_EXP_128) | res.hi
				is_inexact_lt_midpoint = 1
			} else if (eq_half_ulp != 0 && (res.lo&0x01) != 0) || gt_half_ulp != 0 {
				res.lo++
				if res.lo == 0x0 {
					res.hi++
				}
				if (res.hi&MASK_COEFF128) == 0x0001ed09bead87c0 &&
...
func bid_fma_case1ppB_psign_ne_zsign(
	...
	if res.hi != 0x0000314dc6448d93 || res.lo != 0x38c15b0a00000000 { // != 10^33
		if lt_half_ulp != 0 {
			res.hi = z_sign | (z_exp & MASK_EXP_128) | res.hi
			is_inexact_gt_midpoint = 1
		} else if (eq_half_ulp != 0 && (res.lo&0x01) != 0) || gt_half_ulp != 0 {
		...
		*pfpsf |= BID_INEXACT_EXCEPTION
		...
		if e3 > expmin128 {
			if q4 == 1 {
				...
				res.lo = 0x378d8e6400000000 - C4.w0
				z_exp = z_exp - EXP_P1_128
				e3 = e3 - 1
				res.hi = z_sign | (z_exp & MASK_EXP_128) | res.hi
			} else {
				...
				if is_midpoint_lt_even == 0 && is_midpoint_gt_even == 0 &&
					...
					z_exp = z_exp - EXP_P1_128
					e3 = e3 - 1
					res.hi = z_sign | (z_exp & MASK_EXP_128) | 0x0001ed09bead87c0
	...
	if (q3 <= delta && delta < p34 && p34 < delta+q4) || // Case (2)
		...
		if q4 <= 19 {           // 1 <= scale <= 19; C4 fits in 64 bits
			if scale <= 19 { // 10^scale fits in 64 bits
				...
				P128 = __mul_64x64_to_128(C4.w0, bid_ten2k64[scale])
			} else { // 10^scale fits in 128 bits
				// 64 x 128 C4.w[0] * bid_ten2k128[scale - 20]
				P128 = __mul_128x64_to_128(C4.w0, bid_ten2k128[scale-20])
			}
	...
	for {
		if scale == 0 { // this could happen e.g. if we return to case2_repeat
			...
			if scale <= 19 { // 10^scale fits in 64 bits
				...
				*res = __mul_128x64_to_128(C3.lo, bid_ten2k128[scale-20])
			}
		...
		if x0 == 0 { // this could happen only if we return to case2_repeat, or
			...
			R64 = bid_round64_2_18(q4, x0, C4.w0, &incr_exp,
				&is_midpoint_lt_even, &is_midpoint_gt_even,
				&is_inexact_lt_midpoint, &is_inexact_gt_midpoint)
			if incr_exp != 0 {
			...
			if incr_exp != 0 {
				...
				if q4-x0 <= 19 { // 1 <= q4 - x0 <= 19
					R192.w0 = bid_ten2k64[q4-x0]
					// R192.w[1] stays 0
					// R192.w[2] stays 0
				} else { // 20 <= q4 - x0 <= 33
					R192.w0 = bid_ten2k128[q4-x0-20].lo
					R192.w1 = bid_ten2k128[q4-x0-20].hi
					// R192.w[2] stays 0
				}
			}
			R128.hi = R192.w1
			...
			if incr_exp != 0 {
				...
				if q4-x0 <= 19 { // 1 <= q4 - x0 <= 19
					R256.w0 = bid_ten2k64[q4-x0]
					// R256.w[1] stays 0
					// R256.w[2] stays 0
					// R256.w[3] stays 0
				} else { // 20 <= q4 - x0 <= 33
					R256.w0 = bid_ten2k128[q4-x0-20].lo
					R256.w1 = bid_ten2k128[q4-x0-20].hi
					// R256.w[2] stays 0
					// R256.w[3] stays 0
				}
			}
		...
		if z_sign == p_sign {
			lsb = res.lo & 0x01 // lsb of C3 * 10^scale
			...
			lsb = res.lo & 0x01 // lsb of C3 * 10^scale; R128 contains rounded C4
			...
			// increase scale by 1
			if e3 > expmin128 && ((res.hi < 0x0000314dc6448d93 ||
				(res.hi == 0x0000314dc6448d93 &&
					res.lo < 0x38c15b0a00000000)) ||
		...
		if e3 == expmin128 { // and if significand < 10^33 => result is tiny
			if (res.hi&MASK_COEFF128) < 0x0000314dc6448d93 ||
				((res.hi&MASK_COEFF128) == 0x0000314dc6448d93 &&
					res.lo < 0x38c15b0a00000000) {
				is_tiny = 1
			}
			...
			if x0 == ind { // the result before rounding is 0.9... * 10^emin
				...
				// 2 <= ind <= 18, 1 <= x0 <= 17
				R64 = bid_round64_2_18(ind, x0, res.lo, &incr_exp,
					&is_midpoint_lt_even, &is_midpoint_gt_even,
					&is_inexact_lt_midpoint,
					&is_inexact_gt_midpoint)
				if incr_exp != 0 {
...
func bid_fma_case7(
	...
	if is_midpoint_lt_even == 0 && is_midpoint_gt_even == 0 &&
		...
		if p_sign == z_sign {
			...
			if res.hi != 0x0000314dc6448d93 || res.lo != 0x38c15b0a00000000 {
				is_inexact_gt_midpoint = 1
			} else {
				...
				if delta > p34+1 {
					...
					if q3 <= 19 {
						if C3.lo < bid_midpoint64[q3-1] {
							is_inexact_gt_midpoint = 1
						} else if C3.lo == bid_midpoint64[q3-1] {
							is_midpoint_lt_even = 1
						} else {
							res.hi = 0x0001ed09bead87c0
							res.lo = 0x378d8e63ffffffff
						...
						if C3.hi < bid_midpoint128[q3-20].hi ||
							(C3.hi == bid_midpoint128[q3-20].hi &&
								C3.lo < bid_midpoint128[q3-20].lo) {
							is_inexact_gt_midpoint = 1
		...
		if z_sign != p_sign {
			...
			if res.hi == 0x0000314dc6448d93 && res.lo == 0x38c15b09ffffffff {
				res.hi = 0x0001ed09bead87c0
				res.lo = 0x378d8e63ffffffff
				e4 = e4 - 1
			}
	...
	if rnd_mode == BID_ROUNDING_TO_NEAREST && e4 > expmax {
		...
		*pfpsf |= (BID_OVERFLOW_EXCEPTION | BID_INEXACT_EXCEPTION)
		...
		res.hi = p_sign | (p_exp & MASK_EXP_128) | res.hi
	}
	if rnd_mode != BID_ROUNDING_TO_NEAREST {
		bid_rounding_correction(rnd_mode,
			is_inexact_lt_midpoint,
...
func bid_fma_cases_11_12(
	...
	x0 = e4 - e3
	if q3 <= 18 {
		R64 = bid_round64_2_18(q3, x0, C3.lo, &incr_exp,
			&is_midpoint_lt_even, &is_midpoint_gt_even,
			&is_inexact_lt_midpoint, &is_inexact_gt_midpoint)
		C3.lo = R64
	} else if q3 <= 38 {
	...
	if p_sign == z_sign {
		...
		if is_inexact_lt_midpoint != 0 {
			...
			if is_midpoint_lt_even != 0 {
				...
				R256.w0--
				if R256.w0 == 0xffffffffffffffff {
					R256.w1--
					if R256.w1 == 0xffffffffffffffff {
						R256.w2--
	...
	if rnd_mode == BID_ROUNDING_TO_NEAREST {
		...
		bid_rounding_correction(rnd_mode,
			...
			is_midpoint_lt_even, is_midpoint_gt_even,
			0, &P128, pfpsf)
		scale := int((P128.hi&MASK_EXP128)>>49) - 6176
	...
	if e4 < expmin128 {
		...
		if x0 > ind {
			...
			R128.hi = res.hi & MASK_COEFF128
			R128.lo = res.lo
			if ind <= 19 {
				if R128.lo < bid_midpoint64[ind-1] {
					lt_half_ulp = 1
					is_inexact_lt_midpoint = 1
				} else if R128.lo == bid_midpoint64[ind-1] {
					eq_half_ulp = 1
					is_midpoint_gt_even = 1
				} else {
					gt_half_ulp = 1
			...
			res.hi = p_sign | (uint64(e4+6176) << 49) | (res.hi & MASK_COEFF128)
			// avoid double rounding error
			if (is_inexact_gt_midpoint0 != 0 || is_midpoint_lt_even0 != 0) &&
				is_midpoint_lt_even != 0 {
				res.lo--
	...
	if ((res.hi&0x7fffffffffffffff) == 0x0000314dc6448d93 &&
		...
		(((rnd_mode == BID_ROUNDING_TO_NEAREST || rnd_mode == BID_ROUNDING_TIES_AWAY) &&
			...
			((((rnd_mode == BID_ROUNDING_UP) && (res.hi&MASK_SIGN64 == 0)) ||
				((rnd_mode == BID_ROUNDING_DOWN) && (res.hi&MASK_SIGN64 != 0))) &&
				(is_midpoint_lt_even != 0 || is_midpoint_gt_even != 0 ||
					is_inexact_lt_midpoint != 0 || is_inexact_gt_midpoint != 0))) {
		is_tiny = 1
	}
...
func bid_add_and_round(q3, q4, e4, delta, p34 int,
	...
	if scale == 0 {
		...
		R128 = __mul_64x128_to_128(C3.lo, bid_ten2k128[scale-58])
		R256 = __mul_128x128_to_256(R128, bid_ten2k128[18])
	}
	...
	if p_sign == z_sign { // R256 = C4 + R256
		...
		if R256.w3 > C4.w3 || (R256.w3 == C4.w3 && R256.w2 > C4.w2) ||
			(R256.w3 == C4.w3 && R256.w2 == C4.w2 && R256.w1 > C4.w1) ||
			(R256.w3 == C4.w3 && R256.w2 == C4.w2 && R256.w1 == C4.w1 &&
				R256.w0 >= C4.w0) { // C3 * 10^scale >= C4
		...
		if R256.w3 == 0x0 && R256.w2 == 0x0 &&
			R256.w1 == 0x0 && R256.w0 == 0x0 {
			if rnd_mode != BID_ROUNDING_DOWN {
	...
	if ind <= p34 {
		...
		res.hi = p_sign | (uint64(e4+6176) << 49) | R256.w1
		res.lo = R256.w0
	} else { // ind > p34
		x0 = ind - p34
		if ind <= 38 {
		...
		if rnd_mode == BID_ROUNDING_TO_NEAREST {
			...
			scale = int((P128.hi&MASK_EXP_128)>>49) - 6176
			// DECIMAL_TINY_DETECTION_AFTER_ROUNDING = 0 handled above
		}
		ind = p34
		res.hi = p_sign | (uint64(e4+6176) << 49) | R128.hi
		res.lo = R128.lo
	}
...
func bid_bid_nr_digits256(R256 BID_UINT256) int {
	var ind int
	if R256.w3 == 0x0 && R256.w2 == 0x0 && R256.w1 == 0x0 {
		for ind = 1; ind <= 19; ind++ {
			if R256.w0 < bid_ten2k64[ind] {
				break
			}
		}
	} else if R256.w3 == 0x0 && R256.w2 == 0x0 &&
		(R256.w1 < bid_ten2k128[0].hi ||
			(R256.w1 == bid_ten2k128[0].hi && R256.w0 < bid_ten2k128[0].lo)) {
		ind = 20
	} else if R256.w3 == 0x0 && R256.w2 == 0x0 {
		for ind = 1; ind <= 18; ind++ {
			if R256.w1 < bid_ten2k128[ind].hi ||
				(R256.w1 == bid_ten2k128[ind].hi && R256.w0 < bid_ten2k128[ind].lo) {
				break
			}
		}
		ind = ind + 20
	} else if R256.w3 == 0x0 &&
		(R256.w2 < bid_ten2k256[0].w2 ||
			(R256.w2 == bid_ten2k256[0].w2 && R256.w1 < bid_ten2k256[0].w1) ||
			(R256.w2 == bid_ten2k256[0].w2 && R256.w1 == bid_ten2k256[0].w1 &&
				R256.w0 < bid_ten2k256[0].w0)) {
		ind = 39
	} else {
		for ind = 1; ind <= 29; ind++ {
			if R256.w3 < bid_ten2k256[ind].w3 ||
				(R256.w3 == bid_ten2k256[ind].w3 && R256.w2 < bid_ten2k256[ind].w2) ||
				(R256.w3 == bid_ten2k256[ind].w3 && R256.w2 == bid_ten2k256[ind].w2 &&
					R256.w1 < bid_ten2k256[ind].w1) ||
				(R256.w3 == bid_ten2k256[ind].w3 && R256.w2 == bid_ten2k256[ind].w2 &&
					R256.w1 == bid_ten2k256[ind].w1 && R256.w0 < bid_ten2k256[ind].w0) {
				break
			}
		}
		ind = ind + 39
