...
// Bid64ToUint32Ceil is ported mechanically from Intel bid64_to_uint32.c: bid64_to_uint32_ceil.
func Bid64ToUint32Ceil(x uint64) (uint32, uint32) {
	var res uint32
	var x_sign uint64
	var x_exp uint64
	...
	if (q + exp) > 10 { // x >= 10^10 ~= 2^33.2... (cannot fit in 32 bits)
		...
		// under '1 <= q + exp <= 10'
		if x_sign != 0 { // if n < 0 and q + exp = 10 then x is much less than -1
			// => set invalid flag
			pfpsf |= BID_INVALID_EXCEPTION
			// return Integer Indefinite
			res = 0x80000000
			...
			if q <= 11 {
				...
				tmp64 = 0x9fffffff6 * bid_ten2k64[q-11]
				if C1 > tmp64 {
					// set invalid flag
					pfpsf |= BID_INVALID_EXCEPTION
					// return Integer Indefinite
...
// Bid64ToUint32Xceil is ported mechanically from Intel bid64_to_uint32.c: bid64_to_uint32_xceil.
func Bid64ToUint32Xceil(x uint64) (uint32, uint32) {
	var res uint32
	var x_sign uint64
	var x_exp uint64
	var exp int // unbiased exponent
