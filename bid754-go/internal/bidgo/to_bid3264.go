	...
	if uint(expon) > 191 {
		if expon < 0 {
			...
			if rmode == 0 && (coeff&1) != 0 {
				remainder_h := Q.hi & ((uint64(1) << amount) - 1)
				if remainder_h == 0 && Q.lo < bid_reciprocals10_64[extraDigits] {
					coeff--
				}
			}
	...
	if uint(expon) > DECIMAL_MAX_EXPON_32 {
		if expon < 0 {
			...
			if (flags & BID_INEXACT_EXCEPTION) != 0 {
				...
				default:
					// round up
					Stemp, carry = __add_carry_out(Q.lo, bid_reciprocals10_64[extra_digits])
					_ = Stemp
