...
// Bid32NaN returns +QNaN with optional payload from tagp string.
// Ported from bid32_noncomp.c: bid32_nan.
func Bid32NaN(tagp string) uint32 {
	res := uint32(0x7c000000) // +QNaN
	if tagp == "" {
		return res
	}
	x, _ := Bid32FromStringRaw(tagp, BID_ROUNDING_TO_NEAREST)
