...
// Bid64ToUint32Floor is ported mechanically from Intel bid64_to_uint32.c: bid64_to_uint32_floor.
func Bid64ToUint32Floor(x uint64) (uint32, uint32) {
	var res uint32
	var x_sign uint64
	var x_exp uint64
	...
	if x_sign != 0 { // if n < 0 the conversion is invalid
		// set invalid flag
		pfpsf |= BID_INVALID_EXCEPTION
		// return Integer Indefinite
		res = 0x80000000
...
// Bid64ToUint32Xfloor is ported mechanically from Intel bid64_to_uint32.c: bid64_to_uint32_xfloor.
func Bid64ToUint32Xfloor(x uint64) (uint32, uint32) {
	var res uint32
	var x_sign uint64
	var x_exp uint64
	var exp int // unbiased exponent
