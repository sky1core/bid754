...
// Bid64ToInt32Ceil is ported mechanically from Intel bid64_to_int32.c: bid64_to_int32_ceil.
func Bid64ToInt32Ceil(x uint64) (int32, uint32) {
	var res int32
	var x_sign uint64
	var x_exp uint64
	...
	if (q + exp) > 10 { // x >= 10^10 ~= 2^33.2... (cannot fit in 32 bits)
		...
		if x_sign != 0 { // if n < 0 and q + exp = 10
			...
			if q <= 11 {
				...
				tmp64 = 0x50000000a * bid_ten2k64[q-11]
				if C1 >= tmp64 {
					// set invalid flag
					pfpsf |= BID_INVALID_EXCEPTION
					// return Integer Indefinite
			...
			if q <= 11 {
				...
				tmp64 = 0x4fffffff6 * bid_ten2k64[q-11]
				if C1 > tmp64 {
					// set invalid flag
					pfpsf |= BID_INVALID_EXCEPTION
					// return Integer Indefinite
...
// Bid64ToInt32Xceil is ported mechanically from Intel bid64_to_int32.c: bid64_to_int32_xceil.
func Bid64ToInt32Xceil(x uint64) (int32, uint32) {
	var res int32
	var x_sign uint64
	var x_exp uint64
	var exp int // unbiased exponent
