...
// bid32_add_core performs the Intel BID32 addition and returns the status
// raised by that same computation.
func bid32_add_core(x, y uint32, rndMode int) (uint32, uint32) {
	var Tmp BID_UINT128
	var S int64
	var sign_ab int64
	var SU, CB, P, Q, R uint64
	...
	if n_digits <= MAX_FORMAT_DIGITS_32 {
		res = get_BID32_flags(sign_a, exponent_b, P, rndMode, &flags)
		return res, flags
	}
	...
	res = get_BID32_flags(sign_a, exponent_b+extra_digits, Q, rndMode, &flags)
...
func bid32_sub_core(x, y uint32, rndMode int) (uint32, uint32) {
	// negate y if it's not NaN
	if (y & NAN_MASK32) != NAN_MASK32 {
		y ^= 0x80000000
	}
