...
func Bid128Frexp(x BID_UINT128) (BID_UINT128, int) {
	...
	if (x.hi & INFINITY_MASK64) >= INFINITY_MASK64 {
		...
		if (x.hi & 0x6000000000000000) == 0x6000000000000000 {
			exp := 0
			exp_x = uint32((x.hi & MASK_EXP2_128) >> 47) // biased
		...
		// unpack x
		exp_x = uint32((x.hi & MASK_EXP_128) >> 49) // biased
		sig_x.hi = x.hi & MASK_COEFF_128
		sig_x.lo = x.lo
		...
		if sig_x.hi == 0 {
			if sig_x.lo >= 0x0020000000000000 { // z >= 2^53
				...
				if sig_x.lo >= 0x0000000100000000 { // z >= 2^32
					tmp_ui64 := math.Float64bits(float64(sig_x.lo >> 32)) // exact conversion
					x_nr_bits =
						32 + (int((uint32(tmp_ui64>>52))&0x7ff) - 0x3ff)
				} else { // z < 2^32
					tmp_ui64 := math.Float64bits(float64(sig_x.lo)) // exact conversion
					x_nr_bits =
			...
			tmp_ui64 := math.Float64bits(float64(sig_x.hi)) // exact conversion
