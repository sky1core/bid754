...
// bid32_mul_core performs the Intel BID32 multiplication and returns the
// status raised by that same computation.
func bid32_mul_core(x, y uint32, rndMode int) (uint32, uint32) {
	var Tmp BID_UINT128
	var P, Q, R uint64
	var sign_x, sign_y, coefficient_x, coefficient_y, res, flags uint32
	var valid_x, valid_y bool
	var exponent_x, exponent_y, bin_expon_p, amount, n_digits, extra_digits, rmode int
	...
	if extra_digits == 0 {
		res = get_BID32_flags(sign_x^sign_y, exponent_x, P, rndMode, &flags)
		return res, flags
	}
	...
	// The pinned Intel build uses tininess detection before rounding
	// (DECIMAL_TINY_DETECTION_AFTER_ROUNDING=0). Finish through get_BID32_UF
	// even when rounding reaches the minimum normal value so Underflow is kept.
	res = get_BID32_UF(sign_x^sign_y, exponent_x, Q, uint32(R), rndMode, &flags)
