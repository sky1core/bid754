...
// Bid64NaN returns +QNaN with optional payload from tagp string.
// Ported from bid64_noncomp.c: bid64_nan.
func Bid64NaN(tagp string) uint64 {
	res := uint64(0x7c00000000000000) // +QNaN
	if tagp == "" {
		return res
	}
	x, _ := Bid64FromString(tagp, BID_ROUNDING_TO_NEAREST)
	x = x & 0x0003ffffffffffff // valid values fit in 50 bits
	res = res | x
	return res
}
