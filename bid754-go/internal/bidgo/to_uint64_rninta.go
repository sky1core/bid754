...
// Bid64ToUint64Rninta is ported mechanically from Intel bid64_to_uint64.c: bid64_to_uint64_rninta.
func Bid64ToUint64Rninta(x uint64) (uint64, uint32) {
	...
	if (q + exp) > 20 { // x >= 10^20 ~= 2^66.45... (cannot fit in 64 bits)
		...
		// under '1 <= q + exp <= 20'
		if x_sign != 0 { // if n < 0 and q + exp = 20 then x is much less than -1/2
			// => set invalid flag
			pfpsf |= BID_INVALID_EXCEPTION
			// return Integer Indefinite
			res = 0x8000000000000000
...
// Bid64ToUint64Xrninta is ported mechanically from Intel bid64_to_uint64.c: bid64_to_uint64_xrninta.
