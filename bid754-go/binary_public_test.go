...
// bidgoRoundingModeForTest resolves a known-valid RoundingMode to its bidgo
// domain value for computing expected results. bidgoRoundingMode now returns a
// (value, ok) pair; a mode outside the defined constants is a bug in the test
// and fails it.
func bidgoRoundingModeForTest(t *testing.T, mode RoundingMode) int {
	t.Helper()
	m, ok := bidgoRoundingMode(mode)
	if !ok {
		t.Fatalf("bidgoRoundingModeForTest: undefined RoundingMode %d", int(mode))
	}
	return m
}
...
func TestDecimalBIDToDecimal128AndBinary128Ports(t *testing.T) {
	...
	wantBin128, wantBinFlags := bidgo.Bid32ToBinary128(d32.ToUint32(), bidgoRoundingModeForTest(t, RoundNearestEven))
	...
	wantBin128d64, wantBinFlagsd64 := bidgo.Bid64ToBinary128(d64.ToUint64(), bidgoRoundingModeForTest(t, RoundNearestEven))
	...
	wantBin128d128, wantBinFlagsd128 := bidgo.Bid128ToBinary128(decimal128BIDAsBidgo(d128), bidgoRoundingModeForTest(t, RoundNearestEven))
...
func TestDecimalBIDWidthConversionPorts(t *testing.T) {
	...
	want32, want32Flags := bidgo.Bid64ToBid32(d64.ToUint64(), bidgoRoundingModeForTest(t, RoundNearestEven))
	if got32Flags != bidgoExceptionFlags(want32Flags) || got32.ToUint32() != want32 {
	...
	want64n, want64nFlags := bidgo.Bid128ToBid64(decimal128BIDAsBidgo(d128), bidgoRoundingModeForTest(t, RoundNearestEven))
	...
	want32n, want32nFlags := bidgo.Bid128ToBid32(decimal128BIDAsBidgo(d128), bidgoRoundingModeForTest(t, RoundNearestEven))
...
func TestDecimalBIDNarrowingConversionHonorsRoundingMode(t *testing.T) {
	...
	wantToward, _ := bidgo.Bid64ToBid32(d64.ToUint64(), bidgoRoundingModeForTest(t, RoundTowardZero))
