...
func TestDecimalBIDRawParseAndConvenienceParse(t *testing.T) {
	if got, flags := ParseDecimal128BIDRaw("bogus"); got != canonicalQNaN128BID() || flags != FlagInvalidOperation {
		t.Fatalf("ParseDecimal128BIDRaw returned (%q, %v), want canonical qNaN with InvalidOperation", got.String(), flags)
	}
