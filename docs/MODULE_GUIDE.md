...
| `benchcompare-go/` + `benchcompare-rs/` | Standalone comparative benchmarks against `shopspring/decimal` and `rust_decimal` (cost comparison only, not a verification domain, not a product dependency) | `BUILD.md`, each module's README | `make bench-compare-go`, `make bench-compare-rs` (each runs its operand-contract test first); outside `make verify-all` | letting the comparison libraries reach the product modules; using Criterion's anonymous default baseline instead of `pinned`; presenting these rows as correctness evidence |
