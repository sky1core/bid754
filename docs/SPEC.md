...
- `tools/go2rs` is the only permitted generator for the full Rust implementation artifacts under `bid754-rs/src/generated`; alternate Rust implementation generators, direct C-to-Rust implementation generators, and hand-written replacement implementation paths are not acceptable
- Rust implementation quality, idiom, or performance improvements must be made in `tools/go2rs` or its generated support/prelude rules and then regenerated, not by editing generated Rust artifacts or adding a parallel hand-maintained Rust implementation
...
The source repository URL and the Go module namespace are the same identity:
`github.com/sky1core/bid754`.

- root Go module: `github.com/sky1core/bid754` (the `bid-go/` mechanical port
  is a package of this module, not a separate module)
- standalone codec module in the same repository: `github.com/sky1core/bid754/bidcodec`
- `bid-go/cexport` is a repo-internal quarantined module: its legacy stubs are
  build-guarded against linking, and it is never published or consumed as a
  package
