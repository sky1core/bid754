...
2. **Go·Rust·C 세 경로의 hardware-float 시드 융합(FMA) 상태를 일치시킨다.** Go는 spec상
   ...
   플래그에 따른다. 세 경로가 같은 fused/unfused가 되도록 generator 규칙(`tools/go2rs`,
...
| 대상 | 고정 |
|---|---|
| Intel BID C (`scripts/setup_c_libs.sh` 등) | `-O3 -ffp-contract=off`, `BID_SIZE_LONG=8`(64-bit). Intel BID는 upstream makefile이 x86 전용이라 Linux amd64·macOS arm64에서만 빌드되며 Linux arm64에서는 빌드하지 않는다(§1) — Intel BID는 C oracle 전용이므로 운영에 무관 |
...
1. native 검증(FFI bit-compare, readtest, decTest)을 **Intel BID가 빌드되는 macOS arm64 +
   ...
   `.github/workflows/build.yml` 의 native job 매트릭스가 이 CI 요구의 게이트 정의이나,
   workflow 가 아직 리모트에 적용되지 않아 CI 실가동은 열린 요구로 남아 있다.)
...
4. **FP 시드 봉인 = 구조적 봉인 채택**: "현재 흡수되니 회귀 게이트로 감시"가 아니라, 시드
   ...
   시드의 명시적 unfused(`bid-go`는 mechanical-port 포팅 규칙으로, Rust는 go2rs 재생성으로 따라옴;
