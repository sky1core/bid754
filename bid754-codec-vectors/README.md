...
`vectors.json`은 `devtools/testgen_manifest.json`의 `bid_codec_vectors` 설정을 통해 생성된다. 이 파일은 직접 편집하지 않는다.
...
`devtools/cmd/testgen`은 `devtools/internal/testgen`의 독립 BID bit-layout reference codec과
`devtools/testgen_manifest.json`의 seed/count 설정으로 벡터를 생성한다. 프로덕션
`bid754-codec-go` 패키지를 oracle 로 import 하지 않는다.
