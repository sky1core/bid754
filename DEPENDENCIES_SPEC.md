...
`third_party/intel_dfp/lib/.libbid.build-flags` stamp. A missing or mismatched
...
- `third_party/intel_dfp/lib/libbid.a` 는 검증된 v20U4 source tree 에서 현재 머신의 toolchain 으로 빌드한 산출물이어야 한다
...
Legacy `bid-go/cexport` 의 `cexport`, `libbidgo.a`, `libbidgo.h` 는 local build output 이다. 현재 cexport 경로는 quarantine 상태이므로 이 산출물을 source tree 에 체크인하지 않는다. 이 경로를 다시 검증/배포 경로로 승격하려면 먼저 generator 또는 scripted build 로 재현성 경계를 정의하고, 그 뒤에 artifact 정책을 문서화해야 한다.
...
- Go: `go`; standalone package audits create an isolated local git release repository tagged `bidcodec/v0.1.0` (the Go multi-module subdirectory tag convention) and consume `github.com/sky1core/bid754/bidcodec` through normal module version resolution without a local `replace`
- Rust: `cargo`
- Java: `javac`/`java` standard toolchain for the no-external-dependency vector runner; package builds use pinned Gradle plus checked-in `bid-codec-java/gradle.lockfile`, and standalone package audits publish `dev.bid754.bidcodec:bid-codec-java` to an isolated temporary Maven repository
- Python: temporary virtualenv with pinned `pytest` from `bid-codec-py/pyproject.toml`; package audits read the `bid-codec` version from that same `pyproject.toml` instead of hard-coding the wheel/install version
...
- Go: external module smoke plus generated vector audit through a tagged `github.com/sky1core/bid754/bidcodec v0.1.0` module resolved from the isolated audit git repository; local `replace` is not the package-consumer boundary
...
- Java: pinned Gradle via `scripts/run_pinned_gradle.sh`, checked-in dependency lockfile enforcement, clean build of the exact expected library jar plus sources/javadoc jar output, external jar consumer smoke, and generated vector audit against the built jar
...
않고, `cmd/testgen` 이 생성한 `bid-codec-vectors/vectors.json` 를 직접 읽는다.
