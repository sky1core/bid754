...
- 사용자용 빠른 확인: `cd bid754-go && go test ./...` (portable, Go
  toolchain 만 필요). 전체 재현 감사(`make full-audit`)는 다언어/native
...
| `bid754-rs/` | 공개 Rust 구현 (pre-release). 아직 `publish = false` 이고 안정 외부 Rust API 는 미정의이며, 사용자용 API 계층과 crates.io 발행은 2단계 |
| `bid754-rs/libbid-sys/` | repo 내부 FFI 테스트 바인딩 (`publish = false`) |
| `devtools/` | 발행하지 않는 도구 모듈 (생성기, 스크립트, pinned 입력). 태그/의존 대상 아님 |
| `bid754-go/internal/bidgo/cexport/` | 링크가 차단된 quarantine legacy stub |
...
목록은 Makefile 의 `_full-audit` 타깃이며 `docs/BUILD.md` 에 문서화되어 있습니다.
...
- `devtools/generated/go/intel_dfp_tables.go`, `devtools/generated/rust/intel_dfp_tables.rs` 는 Intel BID C 입력에서 생성된 테이블 아티팩트입니다
...
- `devtools/generated/testspec/` 의 `spec_index.json` 과 `readtest/`, `ffi/` 케이스 샤드는 검증 manifest 에서 생성됩니다. Intel `readtest.in` 쪽은 `readtest.h`, `readtest.in`, 저장소에서 실제로 발견되는 BID 생성자/메서드 표면, 문서화된 historical scope rule (`CMP_FUZZYSTATUS - explicit historical skip 함수군 + CMP_EQUALSTATUS`), 그리고 현재 spec-phase exclusion 목록을 함께 읽어 현재 체크인된 BID `readtest` 부분집합을 기계적으로 선택합니다
- 체크인된 Intel readtest 부분집합은 source-driven 이며 generated `readtest.h` function audit 을 포함합니다. 현재 selected/excluded 함수 수는 `docs/TEST_GENERATION_SPEC.md` 와 generated audit artifact 가 정본입니다
- Rust generated readtest dispatch audit 은 selected function 전체를 skip 0 으로 dispatch 하며, `bid32/64/128_fmod` 의 duplicate `CMP_RELATIVEERR` comparator row 를 더해 적용합니다. 수치는 `docs/TEST_GENERATION_SPEC.md` 가 정본입니다
- decTest suite 는 공식 `devtools/tests/*.decTest` 입력에서 파일별 operation 을 스캔해 non-ignored operation 이 현재 supported operation set 안에 있는 파일만 기계적으로 선택합니다. 선택/잔여 파일 수는 `docs/TEST_GENERATION_SPEC.md` 가 정본입니다
- 생성된 native FFI exact bit-compare subset 은 flags 를 노출하는 symbol 의 result 와 `_IDEC_flags` 를 함께 비교하고 `_IDEC_round` symbol 은 rounding mode `0..4` 를 순회합니다. 커버 함수 그룹과 함수/케이스 수는 `docs/TEST_GENERATION_SPEC.md` 가 정본입니다
