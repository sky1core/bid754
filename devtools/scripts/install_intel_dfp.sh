...
# Intel BID 소스를 준비하고 무결성을 검증한다.
# 기존 src 를 재사용하더라도 setup_generation_inputs.sh 가 아카이브 SHA-256 을
# 재검증하고 추출된 트리를 아카이브와 전수 비교하므로, src 존재 여부와 무관하게
# 항상 실행한다. 이렇게 해야 손상/변조된 소스로 libbid.a 를 빌드하지 않는다.
