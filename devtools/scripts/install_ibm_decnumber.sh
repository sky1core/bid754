...
# Reuse a cached archive only when it already matches the pinned sha256; a
# present-but-invalid copy is reported and deleted so the download re-runs
# (never silently validated). The checksum is verified on every path -- cache
# hit or fresh download alike -- and a fresh download that still fails the pin
# is a hard error below.
if [ -f "$DECNUMBER_ARCHIVE" ] && [ "$(sha256_of "$DECNUMBER_ARCHIVE")" != "$DECNUMBER_SHA256" ]; then
    echo "cached archive failed sha256; deleting and re-downloading: $DECNUMBER_ARCHIVE" >&2
    rm -f "$DECNUMBER_ARCHIVE"
fi
...
got_sha="$(sha256_of "$DECNUMBER_ARCHIVE")"
