...
func GenerateDectestTestOutputs(repoRoot string, spec SharedSpec) (map[string][]byte, error) {
	...
	files := map[string][]byte{
		...
		dectestGeneratedStubTestPath: []byte(genmarker.Line("testgen") + `
...
func dectestGeneratedSourceFromTemplate(data []byte) string {
	...
	body = strings.TrimPrefix(body, genmarker.Line("testgen")+"\n\n")
...
func TestGeneratedDectestSuites(t *testing.T) {
	...
	assertGeneratedDectestRuntimeSkipInventory(t, spec.DectestRuntimeSkipInventory)
...
func assertGeneratedDectestRuntimeSkipInventory(t *testing.T, inventories []testspec.GeneratedDectestRuntimeSkipInventory) {
	t.Helper()
	if len(inventories) != len(expectedGeneratedDectestSuiteCoverage) {
		t.Fatalf("generated dectest runtime skip inventory count = %d, want %d", len(inventories), len(expectedGeneratedDectestSuiteCoverage))
	}
	for i, inventory := range inventories {
		expected := expectedGeneratedDectestSuiteCoverage[i]
		if inventory.Suite != expected.Name {
			t.Fatalf("generated dectest runtime skip inventory[%d] suite = %q, want %q", i, inventory.Suite, expected.Name)
		}
		if inventory.Cases != expected.Cases {
			t.Fatalf("generated dectest runtime skip inventory suite %q cases = %d, want %d", inventory.Suite, inventory.Cases, expected.Cases)
		}
		assertGeneratedDectestSkipReasons(t, inventory.Suite, inventory.SkipReasons, expected.SkipReasons)
	}
}
