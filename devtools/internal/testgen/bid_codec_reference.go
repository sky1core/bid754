...
type bidCodecRefComponents struct {
	...
	// Payload is the full BID128 110-bit NaN payload (a value below 10^33),
	// held as a big integer so BID128 payloads at or above 2^64 are preserved
	// exactly instead of being truncated to a low 64-bit field. BID32/BID64
	// payloads are unaffected subsets. A nil or zero Payload means "no payload".
	Payload *big.Int
}
...
// payloadNonZero reports whether a reference-component NaN payload should be
// rendered (a nil or zero payload renders without a suffix).
func payloadNonZero(p *big.Int) bool {
	return p != nil && p.Sign() != 0
}

// payloadLow64 returns the low 64 bits of a NaN payload; BID32/BID64 payloads
// fit entirely in this word. A nil payload is zero.
func payloadLow64(p *big.Int) uint64 {
	if p == nil {
		return 0
	}
	return p.Uint64()
...
// splitPayload128 decomposes a BID128 NaN payload into the encoded low word and
// the 46-bit high-word payload field. A canonical payload is below 10^33 (below
// 2^110), so the high word always fits the 46-bit mask; the mask stays explicit
// as a conservative bound matching the decode side.
func splitPayload128(p *big.Int) (lo, hi uint64) {
	if p == nil || p.Sign() == 0 {
		return 0, 0
	}
	var b [16]byte
	p.FillBytes(b[:])
	hi = binary.BigEndian.Uint64(b[0:8]) & 0x00003fffffffffff
...
func refDecode32(v uint32) bidCodecRefComponents {
	...
	if v&bid32RefNaNMask == bid32RefNaNMask {
		...
		return bidCodecRefComponents{Sign: sign, Kind: kind, Payload: new(big.Int).SetUint64(payload)}
	}
...
func refEncode32(c bidCodecRefComponents) uint32 {
	...
	case bidCodecRefQNaN:
		return sign | bid32RefNaNMask | (uint32(payloadLow64(c.Payload)) & 0x000fffff)
	case bidCodecRefSNaN:
		return sign | bid32RefSNaNMask | (uint32(payloadLow64(c.Payload)) & 0x000fffff)
...
func refEncode64(c bidCodecRefComponents) uint64 {
	...
	case bidCodecRefQNaN:
		return sign | bid64RefNaNMask | (payloadLow64(c.Payload) & 0x0003ffffffffffff)
	case bidCodecRefSNaN:
		return sign | bid64RefSNaNMask | (payloadLow64(c.Payload) & 0x0003ffffffffffff)
	case bidCodecRefZero:
...
func refDecode128(lo, hi uint64) bidCodecRefComponents {
	...
	if hi&bid128RefNaNMask == bid128RefNaNMask {
		...
		if payload.Cmp(bid128RefTen33) >= 0 {
			// Noncanonical NaN payload: normalized to 0, decode-only. Re-encode
			// drops the high payload bits, so the vector stays noncanonical.
			return bidCodecRefComponents{Sign: sign, Kind: kind, Payload: big.NewInt(0)}
		}
		// Preserve the full 110-bit payload (including bits above 2^64), so a
		// high-payload canonical NaN round-trips through refEncode128 below.
...
func refEncode128(c bidCodecRefComponents) (lo, hi uint64) {
	...
	case bidCodecRefQNaN:
		payloadLo, payloadHi := splitPayload128(c.Payload)
		return payloadLo, sign | bid128RefNaNMask | payloadHi
	case bidCodecRefSNaN:
		payloadLo, payloadHi := splitPayload128(c.Payload)
		return payloadLo, sign | bid128RefSNaNMask | payloadHi
	case bidCodecRefZero:
