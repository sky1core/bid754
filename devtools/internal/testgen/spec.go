...
type SharedSpec struct {
	...
	// DectestGoportRuntimeSkipInventory pins the portable Go mechanical-port decTest
	// cross-check leg (TestGeneratedDectestSuitesGoPort): per fixed-width suite it
	// records the raw case total and the mechanical skip-reason buckets, so
	// executed = cases - sum(skip_reasons). General is excluded whole-suite
	// (arbitrary-precision context), so this inventory carries only the fixed-width
	// Decimal32/64/128 suites.
...
type GeneratedReadtestProfileInventory struct {
	...
	// RowsAccepted is the number of readtest.in rows the profile turned into
	// generated cases. RowSkipReasons counts, per named reason, rows that
	// belonged to a selected function but were dropped. Together they make a
	// silent row drop move a reason count instead of only the case total.
