...
func main() {
	...
	// counterpart to the implementation translation above, run in the same go2rs
	// invocation (not a second generator).
	if err := apiemit.Run(root); err != nil {
		fatal("%v", err)
	}

	if err := rejectStaleGeneratedOwnershipMarkers(outDir); err != nil {
		fatal("%v", err)
	}
...
func generateModRs(outDir string, files []string) {
	...
	sb.WriteString(genmarker.Line("go2rs") + "\n\n")
	...
	// The public API surface subtree, emitted by the apiemit subpackage in the
	// same go2rs run. Its lint level is re-tightened inside api/mod.rs so the
	// implementation-module allow does not silence the public surface.
	sb.WriteString("\npub mod api;\n")
...
func rejectStaleGeneratedOwnershipMarkers(outDir string) error {
	entries, err := os.ReadDir(outDir)
	if err != nil {
...
func rejectFinalGeneratedFallbacks(outDir string) error {
	...
	for _, entry := range entries {
		...
		if err := rejectResidualMatchArmBreak(entry.Name(), src); err != nil {
			return err
		}
	}
	return nil
...
// rejectResidualMatchArmBreak fails generation when a generated Rust file
// contains an unlabeled `break` whose nearest enclosing breakable scope is a
// `match` arm rather than a loop.
//
// Background: a Go `switch` case `break` means "leave the switch" (a no-op at
// the end of the case), but Go `switch` bodies convert to Rust `match` arms and
// Rust has no break-out-of-`match`. An unlabeled `break` emitted into a match
// arm therefore silently retargets the nearest enclosing loop, changing control
// flow. The known switch-break shapes are neutralized by the text rewrites
// (reDefaultBreak, rewriteAdd128MatchBreak), which delete the break entirely,
// so a correctly handled switch-break leaves no `break` in the arm. This final
// scan is the strict validation for the case where a new for-loop-nested
// switch-break enters bidgo and the text rewrites miss it: instead of shipping
// a silent semantic change, generation fails. Legitimate loop breaks (nearest
// enclosing breakable scope is a loop) and explicit labeled breaks
// (`break 'label`, e.g. the `'done` block breaks emitted by
// rewriteFmaDoneLabelBlock) are allowed.
func rejectResidualMatchArmBreak(base, code string) error {
	const (
		scopeBlock = iota
		scopeLoop
		scopeArm
	...
	var stack []int
	headKw := ""  // controlling keyword of the current clause (loop/while/for/match)
	prevSig := "" // last significant token; "=>" marks a following match-arm body
	line := 1
	n := len(code)
	...
	// isLifetimeAt reports whether a `'` at index j begins a lifetime/label
	// (e.g. `'done`) rather than a char/byte literal (`'x'`, `'\n'`).
	isLifetimeAt := func(j int) bool {
		if j+1 >= n {
			return false
		}
		if code[j+1] == '\\' {
		...
		if j+2 < n && code[j+2] == '\'' {
			return false // single-char literal 'x'
		}
		return true
	}
	// skipTrivia returns the next significant index at or after j, stepping over
	// whitespace and comments.
	skipTrivia := func(j int) int {
		for j < n {
			c := code[j]
			if c == ' ' || c == '\t' || c == '\r' || c == '\n' {
				j++
	...
	// nearestBreakableIsArm walks the scope stack outward and reports whether the
	// nearest loop-or-arm scope is a match arm (true) or a loop (false, or none).
	nearestBreakableIsArm := func() bool {
		for s := len(stack) - 1; s >= 0; s-- {
			if stack[s] == scopeLoop {
				return false
			}
			if stack[s] == scopeArm {
				return true
	...
	for i := 0; i < n; {
		...
		case c == '\'':
			if isLifetimeAt(i) {
				i++ // consume the quote; the label identifier lexes normally
				prevSig = "'"
			} else if i+1 < n && code[i+1] == '\\' {
				j := i + 3 // skip the quote, backslash, and one escaped char
				for j < n && code[j] != '\'' {
					j++
				}
				i = j + 1
		...
		case c == '{':
			...
			case prevSig == "=>":
				kind = scopeArm
			case headKw == "loop" || headKw == "while" || headKw == "for":
				kind = scopeLoop
			case headKw == "match":
				kind = scopeMatch
			}
		...
		case isIdentStart(c):
			...
			i = j
			switch word {
			case "loop", "while", "for", "match":
				headKw = word
			case "break":
				k := skipTrivia(i)
				labeled := k < n && code[k] == '\'' && isLifetimeAt(k)
				if !labeled && (prevSig == "=>" || nearestBreakableIsArm()) {
					return fmt.Errorf("go2rs left an unlabeled break inside a match arm in %s at line %d: an un-rewritten Go switch-break would silently retarget the enclosing loop in Rust", base, line)
				}
			}
			prevSig = word
		default:
...
func convertParsedFile(fset *token.FileSet, f *ast.File, path string, reg *Registry) (string, error) {
	...
	var sb strings.Builder
	sb.WriteString(genmarker.Line("go2rs from "+filepath.Base(path)) + "\n\n")
	sb.WriteString(mechanicalPortStyleAllowBlock(filepath.Base(path)))
	if needsClippyEqOpAllow(body.String()) {
		sb.WriteString("// Intel BID C's macro-generated comparison logic mechanically ports through\n")
		sb.WriteString("// bid754-go/internal/bidgo/" + filepath.Base(path) + " into a self-comparison here\n")
		sb.WriteString("// (e.g. \"0 == 0\"): a faithful port of the upstream operand shape, not a bug.\n")
		sb.WriteString("// Allow the deny-by-default clippy::eq_op lint instead of rewriting the ported\n")
		sb.WriteString("// comparison, which would diverge this file from its Go/Intel source of truth.\n")
		sb.WriteString("#![allow(clippy::eq_op)]\n\n")
	}
...
// literal tokens only -- not general expressions) of a Rust comparison
// expression. Longer operators are listed before the single-character
// operators they prefix (>=/<= before >/<) so alternation matches the whole
// operator instead of a truncated prefix.
var comparisonOperandsRe = regexp.MustCompile(`\b(\w+)\s*(?:==|!=|<=|>=|<|>)\s*(\w+)\b`)

// needsClippyEqOpAllow reports whether generated Rust source contains a
// comparison whose two bare operands are textually identical (the same
// identifier or integer literal on both sides, e.g. "0 == 0" or "x == x").
// clippy::eq_op is deny-by-default and flags exactly this shape; go2rs finds
// it here (post-translation, on the emitted Rust text) rather than trying to
// special-case it during AST conversion, because the pattern is a faithful
// mechanical port of an upstream Intel BID C macro-expansion artifact
// (confirmed already present verbatim in the bidgo Go source), not something
// go2rs itself introduces. Go's regexp package (RE2) has no backreferences,
// so the two operands are captured separately and compared as strings rather
// than matched with a single "\1"-style pattern.
func needsClippyEqOpAllow(rustSrc string) bool {
	for _, m := range comparisonOperandsRe.FindAllStringSubmatch(rustSrc, -1) {
		if m[1] == m[2] {
			return true
		}
	}
...
// mechanicalPortStyleLints is the fixed set of clippy warn-by-default
// style/complexity/perf lints that the go2rs mechanical port provably triggers
// across the generated bid754-rs/src/generated implementation tree (measured
// from `cargo clippy --locked --all-targets`). Each one is a faithful artifact
// of preserving the upstream Intel BID C / bidgo Go operand, cast, and
// control-flow shape; rewriting the emitted code to satisfy them (dropping
// parentheses or casts, collapsing branches, etc.) would diverge these files
// from their Go/Intel source of truth without changing behavior, which the
// readtest, decTest, and public-API parity verification domains already pin.
//
// The list is an explicit measured set rather than a blanket clippy::all so a
// genuinely NEW lint class still surfaces as a warning instead of being
// silently swallowed. It intentionally excludes the deny-by-default
// clippy::eq_op correctness lint (handled separately by needsClippyEqOpAllow,
// only on files that carry the self-comparison shape) and any lint that fires
// outside this go2rs port tree (e.g. clippy::large_const_arrays in the
// hand-generated table modules, which is not emitted here).
var mechanicalPortStyleLints = []string{
...
// mechanicalPortStyleAllowBlock renders the file-top comment plus the
// crate-module inner attribute that allows mechanicalPortStyleLints. It is
// emitted on every go2rs mechanical port implementation file (the base is the
// bidgo Go source file name), because every such file shares the same rationale
// and the same verification-domain behavior pin.
func mechanicalPortStyleAllowBlock(base string) string {
	var sb strings.Builder
	sb.WriteString("// Mechanical Go/C-shaped port of bid754-go/internal/bidgo/" + base + "; the style/shape\n")
	sb.WriteString("// lints below are faithful artifacts of the upstream operand/cast/control-flow\n")
	sb.WriteString("// shape whose behavior is pinned by the readtest/decTest/public-API parity\n")
	sb.WriteString("// verification domains, so they are allowed here rather than rewritten.\n")
	sb.WriteString("#![allow(\n")
	for _, lint := range mechanicalPortStyleLints {
		sb.WriteString("    " + lint + ",\n")
	}
	sb.WriteString(")]\n\n")
	return sb.String()
...
func convertStmt(fset *token.FileSet, stmt ast.Stmt, src []byte, indent int, namedReturns []namedReturn) string {
	...
	case *ast.BranchStmt:
		...
		case token.FALLTHROUGH:
			// Tagged switches strip trailing fallthrough in convertSwitchStmt
			// and chain case bodies instead. A fallthrough reaching here
			// (e.g. inside an expressionless switch) has no Rust equivalent;
			// emit a fallback marker so rejectGeneratedFallbacks fails the
			// generation instead of silently dropping control flow.
			return fmt.Sprintf("%s// TODO: unhandled fallthrough\n", ind)
		}
		return ""
...
func convertSwitchStmt(fset *token.FileSet, s *ast.SwitchStmt, src []byte, indent int, namedReturns []namedReturn) string {
	...
	// Collect case clauses, stripping the trailing Go fallthrough statement.
	// A clause that falls through executes the next clause's body as well, so
	// the converted body of clause i is body(i) followed by the converted
	// body of clause i+1 (chained transitively for consecutive fallthroughs).
	...
	for _, stmt := range s.Body.List {
		...
		clauses = append(clauses, switchClause{cc: cc, body: body, fallsThrough: falls})
	}

	// Convert bodies back-to-front so fallthrough chains onto the already
	// converted next body.
	converted := make([]string, len(clauses))
	for i := len(clauses) - 1; i >= 0; i-- {
		var body strings.Builder
		for _, bodyStmt := range clauses[i].body {
			body.WriteString(convertStmt(fset, bodyStmt, src, indent+2, namedReturns))
		}
		if clauses[i].fallsThrough && i+1 < len(clauses) {
			body.WriteString(converted[i+1])
		}
		converted[i] = body.String()
	}

	// Go's default clause matches "no case matched" regardless of its source
	// position, but Rust match arms are tried top-down, so emitting the `_`
	// arm at a mid-switch source position would shadow every arm after it.
	// The `_` arm is therefore always emitted last.
	defaultIdx := -1
	for i, clause := range clauses {
		if clause.cc.List == nil {
			defaultIdx = i
		}
	}
	// Conservative check: when a mid-position default participates in a
	// fallthrough relation (a case falling through into the default, or the
	// default falling through into the next case), correct emission would
	// depend on the body-chaining details rather than on arm order alone.
	// bidgo contains no such pattern, so reject it explicitly instead of
	// reordering silently; the marker fails rejectGeneratedFallbacks.
	if defaultIdx >= 0 && defaultIdx < len(clauses)-1 {
		if clauses[defaultIdx].fallsThrough ||
			(defaultIdx > 0 && clauses[defaultIdx-1].fallsThrough) {
			return fmt.Sprintf("%s// TODO: unhandled mid-switch default with fallthrough\n", ind)
		}
	}
	...
	for i, clause := range clauses {
		cc := clause.cc
		if cc.List == nil {
			// default: emitted last, after every case arm
			continue
		}
		conds := pendingConds
		...
		// A bare `case A: fallthrough` into a non-default case merges
		// into the next arm's pattern (`A | B => ...`).
		if clause.fallsThrough && len(clause.body) == 0 &&
			i+1 < len(clauses) && clauses[i+1].cc.List != nil {
			pendingConds = conds
			continue
		}
		pendingConds = nil
		sb.WriteString(fmt.Sprintf("%s    %s => {\n", ind, strings.Join(conds, " | ")))
		sb.WriteString(converted[i])
		sb.WriteString(fmt.Sprintf("%s    }\n", ind))
	}
	if defaultIdx >= 0 {
		sb.WriteString(fmt.Sprintf("%s    _ => {\n", ind))
		sb.WriteString(converted[defaultIdx])
		sb.WriteString(fmt.Sprintf("%s    }\n", ind))
	} else {
		sb.WriteString(fmt.Sprintf("%s    _ => {}\n", ind))
	}
