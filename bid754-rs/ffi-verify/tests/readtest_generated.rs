...
// Source: devtools/tools/registry/readtest_specs.json + devtools/third_party/intel_dfp/TESTS/readtest.in
...
fn dispatch(func_name: &str, parts: &[&str], rm: i64, ulp_add: f64) -> DispatchResult {
    match func_name {
        ...
        "bid128_nextdown" => {
            dispatch_unary_bid128_optional_ignored_arg(parts, rm, bid128_next_down)
        },
        "bid128_nexttoward" => {
            if parts.len() <= 5 { return DispatchResult::Skip; }
            let Some(a0) = parse_bid128(parts[2]) else { return DispatchResult::Skip };
        ...
        "bid128_nextup" => {
            dispatch_unary_bid128_optional_ignored_arg(parts, rm, bid128_next_up)
        },
        "bid128_quantexp" => {
            if parts.len() <= 4 { return DispatchResult::Skip; }
            let Some(a0) = parse_bid128(parts[2]) else { return DispatchResult::Skip };
        ...
        "bid128_to_string" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[3], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[4]);
            let got_string = bid128_to_string(a0);
            let (got, round_trip_flags) = bid128_from_string_via_c_readtest(&got_string, rm);
            let result = compare_bid128(got, expected, round_trip_flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid128dd_add" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dd_add(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128dd_div" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dd_div(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128dd_mul" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dd_mul(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid128dq_add" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dq_add(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128dq_div" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dq_div(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128dq_mul" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dq_mul(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128dq_sub" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128dq_sub(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid128qd_add" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128qd_add(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128qd_div" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128qd_div(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid128qd_mul" => {
            ...
            let Some(expected) = parse_bid128_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid128qd_mul(a0, a1, rm);
            let result = compare_bid128(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid32_add" => {
            ...
            let Some(expected) = parse_bid32_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
        ...
        "bid32_fma" => {
            ...
            let Some(expected) = parse_bid32_expected(parts[5], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[6]);
        ...
        "bid32_to_string" => {
            ...
            let Some(expected) = parse_bid32_expected(parts[3], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[4]);
            let got_string = bid32_to_string(a0);
            let (got, round_trip_flags) = bid32_from_string_via_c_readtest(&got_string, rm);
            let result = compare_u32(got, expected, round_trip_flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid64_to_string" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[3], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[4]);
            let got_string = bid64_to_string(a0);
            let (got, round_trip_flags) = bid64_from_string_via_c_readtest(&got_string, rm);
            let result = compare_u64(got, expected, round_trip_flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid64dq_add" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64dq_add(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64dq_div" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64dq_div(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64dq_mul" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64dq_mul(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid64qd_add" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qd_add(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64qd_div" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qd_div(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64qd_mul" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qd_mul(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64qd_sub" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qd_sub(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        ...
        "bid64qq_add" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qq_add(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64qq_div" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qq_div(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
        "bid64qq_mul" => {
            ...
            let Some(expected) = parse_bid64_expected(parts[4], rm) else { return DispatchResult::Skip };
            let expected_flags = parse_flags(parts[5]);
            let (got, flags) = bid64qq_mul(a0, a1, rm);
            let result = compare_u64(got, expected, flags, expected_flags, CmpMode::CmpFuzzy, rm, ulp_add);
            if !matches!(result, DispatchResult::Pass) { return result; }
            DispatchResult::Pass
        },
...
fn test_readtest_generated_decimal32() {
    ...
    assert_eq!(s.passed, 20875, "decimal32 readtest passed-case count derived from readtest.in at generation time");
}
...
fn test_readtest_generated_status_control() {
    ...
    assert_eq!(s.passed, 137, "status-control readtest passed-case count derived from readtest.in at generation time");
}
