// Auto-generated from to_int32_ceil.go by go2rs. Do not edit.

use super::prelude::*;

pub fn bid64_to_int32_ceil(mut x: u64) -> (i32, u32) {
    let mut res: i32 = 0;
    ...
    if (((q.wrapping_add(exp))) > 10) {
        ...
        if (x_sign != 0) {
            if (q <= 11) {
                ...
                if (tmp64 >= 0x50000000a) {
                    pfpsf |= 1;
                    res = (-0x80000000);
                    return (res, pfpsf);
                }
            } else {
                tmp64 = ((0x50000000a as u64).wrapping_mul(bid_ten2k64[(q.wrapping_sub(11)) as usize]));
                if (C1 >= tmp64) {
                    pfpsf |= 1;
                    res = (-0x80000000);
                    return (res, pfpsf);
            ...
            if (q <= 11) {
                tmp64 = (C1.wrapping_mul(bid_ten2k64[((11 as i64).wrapping_sub(q)) as usize]));
                if (tmp64 > 0x4fffffff6) {
                    pfpsf |= 1;
                    res = (-0x80000000);
                    return (res, pfpsf);
                }
            } else {
                tmp64 = ((0x4fffffff6 as u64).wrapping_mul(bid_ten2k64[(q.wrapping_sub(11)) as usize]));
                if (C1 > tmp64) {
                    pfpsf |= 1;
                    res = (-0x80000000);
                    return (res, pfpsf);
    ...
    if (((q.wrapping_add(exp))) <= 0) {
        ...
        if (exp < 0) {
            ...
            if (x_sign != 0) {
                res = ((Cstar as i32).wrapping_neg());
            } else {
                res = (Cstar as i32);
            }
        } else if (exp == 0) {
            if (x_sign != 0) {
                res = ((C1 as i32).wrapping_neg());
            } else {
                res = (C1 as i32);
            }
        } else {
            if (x_sign != 0) {
                res = (((C1.wrapping_mul(bid_ten2k64[exp as usize])) as i32).wrapping_neg());
            } else {
                res = ((C1.wrapping_mul(bid_ten2k64[exp as usize])) as i32);
            }
        }
    }
    return (res, pfpsf);
...
pub fn bid64_to_int32_xceil(mut x: u64) -> (i32, u32) {
    let mut res: i32 = 0;
    let mut x_sign: u64 = 0;
    let mut x_exp: u64 = 0;
    let mut exp: i64 = 0;
