...
//!
//! This embedded module backs this crate's generated vector verification path
//! and mirrors the standalone `bid754-codec-rs` contract (codec logic
//! intentionally exists twice per language; the shared generated vectors are
//! the gate that keeps the two copies equivalent): `encode32`/`encode64`/
//! `encode128` are validating packing APIs that reject out-of-range fields
//! through `Result` instead of silently truncating, masking, or clamping, and
//! `from_string` accepts the same strict ASCII grammar with the same
//! schema-wide limits (coefficient below `10^34`, NaN payload below `10^33`).
...
mod tests {
    ...
    fn test_encode_boundaries() {
        ...
        // payload uses the full u128, so its 10^33 boundary is exercised.
        ...
        // Those reject records are skipped with the type system reported as the
        // constraint, matching the cross-language schema note.
    }
