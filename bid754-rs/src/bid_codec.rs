...
pub fn from_string(s: &str) -> Result<Components, String> {
    ...
    if let Some(rest) = upper.strip_prefix("SNAN") {
        let payload = if rest.is_empty() {
            ...
            rest.parse::<u64>()
                .map_err(|e| alloc::format!("invalid payload: {}", e))?
        };
    ...
    let exp_part: i32 = if i < bytes.len() && (bytes[i] == b'E' || bytes[i] == b'e') {
        ...
        exp_str
            .parse::<i32>()
            .map_err(|e| alloc::format!("invalid exponent: {}", e))?
    } else {
        0
    };
    ...
    let exponent = exp_part + exp_adjust;
...
mod tests {
    ...
    fn test_to_string() {
        ...
        assert_eq!(
            to_string(&Components::new_nan(false, Kind::QNaN, 0)),
            "+NaN"
        );
        assert_eq!(
            to_string(&Components::new_nan(false, Kind::SNaN, 123)),
            "+SNaN123"
        );
        assert_eq!(to_string(&Components::new_zero(false, 0)), "+0");
        assert_eq!(to_string(&Components::new_zero(true, -5)), "-0E-5");
        assert_eq!(
            to_string(&Components::new_normal(false, 12345, -2)),
            "+1.2345E+2"
        );
